...
\begin{equation}
    I_{0+} = (51.0 \pm 0.5)\,\mu\mathrm{A}
    \qquad \text{und} \qquad
    I_{0-} = (-48.8 \pm 0.5)\,\mu\mathrm{A}.
\end{equation}
...
\begin{equation}
    2I_C
    = \frac{|I_{0+}|+|I_{0-}|}{2}
    = \frac{51.0 + 48.8}{2}\,\mu\mathrm{A}
    = (49.9 \pm 0.4)\,\mu\mathrm{A}.
\end{equation}
...
\begin{equation}
    I_C R_N
    = (24.95 \pm 0.20)\mu\mathrm{A}\cdot (4.58 \pm 0.06)\,\Omega
    = (114.3 \pm 1.8)\mu\mathrm{V}.
\end{equation}

Der erhaltene Wert liegt im erwarteten Bereich von etwa $100$ bis $200\mu\mathrm{V}$. Die Messung ist damit gut mit der erwarteten Größenordnung für den verwendeten SQUID-Sensor verträglich. Die Unsicherheiten ergeben sich aus den Fit- bzw. Ableseunsicherheiten der Achsenschnittpunkte und der Steigung der linearen Kennlinienbereiche.
...
\begin{equation}
    \frac{e}{h} = \frac{44 \cdot 10^9\,\mathrm{Hz}}{2 \cdot 88.625 \cdot 10^{-6}\,\mathrm{V}} = 2.48 \pm 0.03 \cdot 10^{-14} \mathrm{Hz/V}
\end{equation}
...
\begin{equation}
    \Delta I = (93.3 \pm 0.4)\,\mu\mathrm{A}.
\end{equation}

Eine vollständige Periode der $V$-$\Phi$-Kennlinie entspricht einer Änderung des magnetischen Flusses um genau ein Flussquant $\Phi_0$. Damit folgt für die Kopplungsinduktivität der internen Rückkoppelspule bezüglich des SQUID-Rings

\begin{equation}
    M = \frac{\Phi_0}{\Delta I}
    = \frac{2.07\times 10^{-15}\,\mathrm{Vs}}{93.3\times 10^{-6}\,\mathrm{A}}
    = (22.16 \pm 0.10)\,\mathrm{pH}.
\end{equation}

Hierbei ist zu beachten, dass diese Größe $M$ nicht die Eigeninduktivität des SQUID-Rings ist, sondern die Kopplungsinduktivität zwischen Rückkoppelspule und SQUID. Für die Eigeninduktivität des SQUID-Rings wird im Mr.-SQUID-Manual näherungsweise $ L \approx 73\,\mathrm{pH} $ angegeben \cite{mr_squid}. Diese setzt sich aus der geometrischen Induktivität des SQUID-Schlitzes, der kinetischen Induktivität des supraleitenden Körpers und der Induktivität der Josephson-Brücken zusammen.

Aus dem Sinusfit wurde für die Amplitude der Spannungsmodulation $A = (10.8 \pm 0.2)\,\mu\mathrm{V}$ bestimmt. Da die maximale Spannungsmodulation $\Delta V$ als Peak-to-Peak-Wert zu verstehen ist, gilt $\Delta V = 2A = (21.6 \pm 0.4)\mu\mathrm{V}$.

Der Modulationsparameter $\beta_L$ kann nun mit zwei verschiedenen Methoden bestimmt werden. Zunächst ergibt sich aus der Definition
...
mit $L=73\,\mathrm{pH}$ und $I_C=(24.95\pm0.20)\,\mu\mathrm{A}$

\begin{equation}
    \beta_{L,\mathrm{ind}}
    = \frac{2\cdot 73\times 10^{-12}\,\mathrm{H}\cdot 24.95\times 10^{-6}\,\mathrm{A}}
    {2.07\times 10^{-15}\,\mathrm{Vs}}
    = 1.76 \pm 0.01.
\end{equation}

Alternativ kann $\beta_L$ aus der gemessenen Spannungsmodulation $\Delta V$ bestimmt werden. Ohne Berücksichtigung thermischer Fluktuationen gilt näherungsweise

\begin{equation}
    \beta_{L,1}
    = \frac{4I_C R_N}{\pi \Delta V} - 1.
\end{equation}
...
\begin{equation}
    \beta_{L,1}
    = \frac{4\cdot 24.95\times10^{-6}\,\mathrm{A}\cdot 4.58\,\Omega}
    {\pi\cdot 21.6\times10^{-6}\,\mathrm{V}} - 1
    = 5.74 \pm 0.16.
\end{equation}

Da der SQUID bei der relativ hohen Temperatur von etwa $77\mathrm{K}$ betrieben wird, sind thermische Fluktuationen nicht vernachlässigbar. Das Mr.-SQUID-Manual \cite{mr_squid} gibt hierfür eine korrigierte Näherung an:

\begin{equation}
    \beta_{L,2}
    =
    \frac{4I_C R_N}{\pi \Delta V}
    \left(
    1 - 3.57\frac{\sqrt{k_BTL}}{\Phi_0}
    \right)
    - 1.
\end{equation}

Mit $T=77\,\mathrm{K}$, $L=73\,\mathrm{pH}$ und $k_B=1.38\times10^{-23}\,\mathrm{J/K}$ ergibt sich

\begin{equation}
    \beta_{L,2}
    =
    \frac{4\cdot 24.95\times10^{-6}\,\mathrm{A}\cdot 4.58\,\Omega}
    {\pi\cdot 21.6\times10^{-6}\,\mathrm{V}}
    \left(
    1 - 3.57
    \frac{\sqrt{1.38\times10^{-23}\,\mathrm{J/K}\cdot 77\,\mathrm{K}\cdot 73\times10^{-12}\,\mathrm{H}}}
    {2.07\times10^{-15}\,\mathrm{Vs}}
    \right)
    - 1
    = 2.50 \pm 0.08.
\end{equation}

Die beiden aus der Spannungsmodulation bestimmten Werte liegen deutlich über dem aus $L$ und $I_C$ berechneten Wert. Dies ist plausibel, da die Näherungen ideale und symmetrische Josephson-Kontakte voraussetzen und die gemessene Spannungsmodulation stark von Rauschen, Drift, der Wahl des Arbeitspunktes und thermischen Fluktuationen beeinflusst wird. Die thermisch korrigierte Methode liefert dabei einen kleineren und physikalisch sinnvolleren Wert als die unkorregierte Näherung.
...
Zusätzlich konnte aus der $V$-$\Phi$-Charakteristik die periodische Abhängigkeit der SQUID-Spannung vom magnetischen Fluss nachgewiesen werden. Für die Spannungsmodulation ergab sich ein Peak-to-Peak-Wert von

\begin{equation}
    \Delta V = (21.6 \pm 0.4)\,\mu\mathrm{V}.
\end{equation}

Aus der Periodendauer der Modulation wurde die Kopplungsinduktivität der Rückkoppelspule zu

\begin{equation}
    M = (22.16 \pm 0.10)\,\mathrm{pH}
\end{equation}

bestimmt. Damit konnte gezeigt werden, dass bereits sehr kleine Stromänderungen in der Rückkoppelspule einer Änderung des magnetischen Flusses um ein Flussquant entsprechen. Unter Verwendung der Eigeninduktivität des SQUID-Rings von $L\approx 73\,\mathrm{pH}$ ergab sich für den Modulationsparameter zunächst

\begin{equation}
    \beta_{L,\mathrm{ind}} = 1.76 \pm 0.01.
\end{equation}

Aus der gemessenen Spannungsmodulation folgten außerdem

\begin{equation}
    \beta_{L,1}=5.74\pm0.16
\end{equation}

ohne thermische Korrektur sowie

\begin{equation}
    \beta_{L,2}=2.50\pm0.08
\end{equation}

mit Berücksichtigung thermischer Fluktuationen. Die Abweichung der verschiedenen $\beta_L$-Werte zeigt, dass die reale SQUID-Charakteristik deutlich durch thermisches Rauschen, Asymmetrien der Josephson-Kontakte und die Wahl des Arbeitspunktes beeinflusst wird. Der thermisch korrigierte Wert liegt näher an der aus $L$ und $I_C$ bestimmten Größenordnung und ist daher als physikalisch plausibler einzuschätzen.

Insgesamt bestätigen die Messungen die zentralen Eigenschaften eines DC-SQUIDs: den Josephson-Effekt, die Flussquantisierung und die hohe Empfindlichkeit gegenüber kleinsten Änderungen des magnetischen Flusses. Die bestimmten Größen $I_C$, $R_N$, $I_CR_N$, $\Delta V$, $M$ und $\beta_L$ liegen in einer sinnvollen Größenordnung und zeigen, dass der verwendete Hochtemperatur-SQUID als empfindlicher Magnetfeldsensor arbeitet. Damit konnten sowohl die theoretischen Grundlagen der makroskopischen Quantenphänomene in Supraleitern als auch deren praktische Anwendung in Form eines SQUID-Sensors experimentell nachvollzogen werden.
