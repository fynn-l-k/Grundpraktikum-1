...
Für die Aufnahme der Spektren wurde zunächst in größeren Schritten über einen größeren Wellenlängenbereich gemessen, um die ungefähre Lage der Emissionsmaxima zu bestimmen. Anschließend wurden gezielt feinere Messungen in den relevanten Bereichen aufgenommen, um die Intensität und die Peakpositionen genauer zu bestimmen. Diese Vorgehensweise war besonders sinnvoll, weil die Positionen der Maxima teilweise bereits aus der ersten groben Messung bekannt waren und dadurch nur noch die empfindlichsten Bereiche genauer vermessen werden mussten. Dadurch konnte der Messzeitaufwand reduziert werden, ohne die Auswertung der spektralen Strukturen zu beeinträchtigen.
...
Das FWHM wurde aus dem Fitparametern der Standardabweichung einer Gauß-Fitfunktion bestimmt. Es gilt folgende Relation zwischen Standardabweichung $\sigma$ und FWHM.
\begin{equation}
...
\end{equation}
Für die rote Ga(As,P)-Diode wird die Emission als Übergang vom Leitungsband zu einem Akzeptorniveau betrachtet \cite[S.~20]{manual}. In guter Näherung wird für die Akzeptorenergie der GaAs-Wert verwendet. Für die Ionisierungsenergie gilt allgemein
\begin{equation}
    E_A = \frac{m_p}{m}\frac{E_H}{\varepsilon^2}.
\end{equation}
Mit $m_p=0.50m$, $\varepsilon=12.9$ und $E_H=\SI{13.6}{\electronvolt}$ \cite[Tabelle~2]{manual} folgt dann
\begin{equation}
...
Für den direkten Übergang ~\eqref{eq:gaasp_direct} gilt allgemein
\begin{equation}
    E_{g,d}(x)=E_{g,d}(0)+\left[E_{g,d}(1)-E_{g,d}(0)-0.23\right]x+0.23x^2.
\end{equation}
Einsetzen der Zahlenwerte $E_{g,d}(0)=\SI{1.43}{\electronvolt}$ und $E_{g,d}(1)=\SI{2.781}{\electronvolt}$ ergibt dann
\begin{equation}
...
Die Differenz ist mit Donator-Akzeptor-Rekombinationen im Si-dotierten GaAs vereinbar. Silizium kann in GaAs amphoter eingebaut werden: Auf einem Ga-Platz wirkt es als Donator, auf einem As-Platz als Akzeptor \cite[S.~18]{manual}. Für die Ionisierungsenergien gilt allgemein
\begin{equation}
    E_D = \frac{m_n}{m}\frac{E_H}{\varepsilon^2},
    \qquad
    E_A = \frac{m_p}{m}\frac{E_H}{\varepsilon^2}.
\end{equation}
Mit $m_n=0.067m$, $m_p=0.50m$ und $\varepsilon=12.9$ \cite[Tabelle~2]{manual} erhält man dann
\begin{align}
...
Die bei flüssiger Stickstofftemperatur aufgenommenen Spektren dienen primär dem Vergleich mit den
Raumtemperaturmessungen. Durch die verringerte thermische Verbreiterung werden die Emissionslinien
schmaler und die Bandkante besser aufgelöst. Die Bestimmung des Aluminiumanteils $x$ erfolgt jedoch
anhand der Raumtemperaturmessung, da die verwendete empirische Beziehung für diese Temperatur gilt.

\begin{figure}[H]
    \centering
    \includegraphics[width=0.75\linewidth]{figures/plot_aufgabe_8_1127.pdf}
    \caption{Spektrum der Probe 1127 bei \SI{293.15}{\kelvin} und \SI{88.75}{\kelvin}. Die Maxima
...
\begin{figure}[H]
    ...
    \caption{Spektrum der Probe 87063 bei \SI{293.15}{\kelvin} und \SI{88.25}{\kelvin}. Die Maxima
    liegen bei \SI{651.0}{\nano\meter} für die warme Messung sowie bei \SI{621.0}{\nano\meter} und
    \SI{628.0}{\nano\meter} für die kalte Messung.}
    \label{fig:aufgabe_8_87063}
\end{figure}

Zur eindeutigen Zuordnung werden die beiden Messreihen nach den Dateibezeichnungen der Abbildungen
als Probe 1127 und Probe 87063 bezeichnet. Beide Proben zeigen beim Abkühlen auf flüssige-Stickstoff-
Temperatur eine Verschiebung der Lumineszenz zu kürzeren Wellenlängen. Dies entspricht einer höheren
Photonenenergie und ist qualitativ mit der Zunahme der Bandlücke bei sinkender Temperatur vereinbar.

Bei Raumtemperatur wird die Emission näherungsweise als Band-Band-Rekombination beschrieben. Nach
der Eagles-Verteilung liegt das Maximum bei $E_g+k_BT/2$ \cite[S.~24]{manual}. Deshalb gilt
\begin{equation}
...
\begin{align}
    h\nu_\mathrm{warm} &= \frac{hc}{\lambda}
    = \frac{\SI{1239.84}{\electronvolt\nano\meter}}{\SI{651.0}{\nano\meter}}
    \approx \SI{1.905}{\electronvolt},\\
    E_{g,d}(x,T_\mathrm{warm}) &\approx \SI{1.892}{\electronvolt},\\
    x_{87063} &\approx 0.328.
\end{align}
Probe 87063 besitzt damit den größeren Aluminiumanteil und entsprechend die größere Bandlücke, was mit
ihrer kürzeren Emissionswellenlänge übereinstimmt. Beide Werte liegen unterhalb von $x_s\approx0.43$,
sodass die Proben im direkten Bereich des Mischsystems liegen \cite[S.~23--26]{manual}.

Für die Auswertung der Tieftemperaturspektren werden nach Abschnitt 2.5.6 die dort auftretenden
Donator-Akzeptor-Paarübergänge herangezogen. Unter Vernachlässigung des Coulomb-Terms $E_C$ gilt nach
Gleichung~\eqref{eq:dap}
\begin{equation}
    h\nu_\mathrm{kalt,2} \approx h\nu_\mathrm{kalt,1} - E_D - E_A,
    \label{eq:dap_diff}
\end{equation}
wobei der kurzwelligere, hochenergetische Peak $\lambda_\mathrm{kalt,1}$ näherungsweise der Bandkante
entspricht ($E_g\approx h\nu_\mathrm{kalt,1}$) und der langwelligere Peak $\lambda_\mathrm{kalt,2}$ dem
Donator-Akzeptor-Paarübergang zugeordnet wird. Damit lässt sich die Störstellenenergie direkt aus der
Differenz der beiden gemessenen Photonenenergien abschätzen, ohne dass eine gesonderte Extrapolation
der Bandlücke auf die tiefe Temperatur notwendig ist:
\begin{equation}
    E_D+E_A \approx hc\left(\frac{1}{\lambda_\mathrm{kalt,1}}-\frac{1}{\lambda_\mathrm{kalt,2}}\right).
    \label{eq:stoerstellen_abschaetzung}
\end{equation}

Bei Probe 1127 ergibt sich mit $\lambda_\mathrm{kalt,1}=\SI{672.1}{\nano\meter}$ und
$\lambda_\mathrm{kalt,2}=\SI{690.5}{\nano\meter}$ (gemessen bei \SI{88.75}{\kelvin})
\begin{align}
    h\nu_\mathrm{kalt,1} &= \frac{\SI{1239.84}{\electronvolt\nano\meter}}{\SI{672.1}{\nano\meter}}
    \approx \SI{1.845}{\electronvolt},\\
    h\nu_\mathrm{kalt,2} &= \frac{\SI{1239.84}{\electronvolt\nano\meter}}{\SI{690.5}{\nano\meter}}
    \approx \SI{1.796}{\electronvolt},\\
    E_D+E_A &\approx \SI{1.845}{\electronvolt}-\SI{1.796}{\electronvolt}
    \approx \SI{49}{\milli\electronvolt}.
\end{align}

Bei Probe 87063 ergibt sich mit $\lambda_\mathrm{kalt,1}=\SI{621.0}{\nano\meter}$ und
$\lambda_\mathrm{kalt,2}=\SI{628.0}{\nano\meter}$ (gemessen bei \SI{88.25}{\kelvin})
\begin{align}
    h\nu_\mathrm{kalt,1} &= \frac{\SI{1239.84}{\electronvolt\nano\meter}}{\SI{621.0}{\nano\meter}}
    \approx \SI{1.997}{\electronvolt},\\
    h\nu_\mathrm{kalt,2} &= \frac{\SI{1239.84}{\electronvolt\nano\meter}}{\SI{628.0}{\nano\meter}}
    \approx \SI{1.974}{\electronvolt},\\
    E_D+E_A &\approx \SI{1.997}{\electronvolt}-\SI{1.974}{\electronvolt}
    \approx \SI{23}{\milli\electronvolt}.
\end{align}

Mittels der im Manuskript angegebenen Formeln für die Ionisierungsenergien der Akzeptoren lassen sich
für die bestimmten Molanteile die in Tabelle~\ref{tab:ionisierung} zusammengefassten theoretischen
Werte berechnen:

\begin{table}[H]
\centering
\caption{Theoretische Ionisierungsenergien verschiedener Akzeptoren für die Molanteile der beiden Proben.}
\label{tab:ionisierung}
\begin{tabular}{lccccc}
\toprule
Molanteil & $E_A(\mathrm{Zn})$ [meV] & $E_A(\mathrm{C})$ [meV] & $E_A(\mathrm{Mg})$ [meV] & $E_A(\mathrm{Si})$ [meV] & $E_A(\mathrm{Ge})$ [meV]\\
\midrule
$x_{87063}\ (0.328)$ & 50.76 & 30.34 & 32.36 & 60.98 & 85.47\\
$x_{1127}\ (0.226)$ & 37.93 & 28.24 & 29.27 & 48.64 & 62.39\\
\bottomrule
\end{tabular}
\end{table}

Der für Probe 1127 ermittelte Wert von \SI{49}{\milli\electronvolt} stimmt sehr gut mit der theoretischen Ionisierungsenergie von Silizium
($E_A(\mathrm{Si})\approx\SI{48.64}{\milli\electronvolt}$) überein und liegt deutlich über den Werten
für Zn, C und Mg sowie unter dem Wert für Ge. Dies legt nahe, dass Probe 1127 mit Silizium dotiert ist.
Die verbleibende Diskrepanz lässt sich durch Messungenauigkeiten der Peaklagen, die Vernachlässigung
der Donatorbindungsenergie $E_D$ im Vergleich zu $E_A$ sowie den vernachlässigten Coulomb-Term $E_C$
erklären.

Für Probe 87063 ergibt sich mit \SI{23}{\milli\electronvolt} hingegen kein befriedigender Vergleichswert: Selbst die niedrigste theoretische Energie
(C-Akzeptor, \SI{30.34}{\milli\electronvolt}) liegt bereits merklich über dem experimentellen Wert, und
alle übrigen Akzeptoren (Mg, Zn, Si, Ge) liegen noch deutlich weiter darüber. Eine eindeutige
Zuordnung zu einem der aufgeführten Akzeptoren ist für diese Probe daher nicht möglich. Die
Diskrepanz lässt sich durch den vernachlässigten Coulomb-Term $E_C$, durch Messungenauigkeiten bei der
Peakbestimmung sowie durch eine mögliche Donatorbindungsenergie $E_D$, die in der einfachen Abschätzung
nicht von $E_A$ getrennt wird, erklären. Insbesondere ein nicht zu vernachlässigender Coulomb-Term
$E_C>0$ würde den tatsächlichen Wert von $E_D+E_A$ nach oben korrigieren und die Übereinstimmung mit
den Manual-Werten verbessern.

\begin{table}[H]
\centering
\caption{Zusammenfassung der charakteristischen Wellenlängen, Aluminiumanteile und abgeschätzten
Störstellenenergien.}
...
Probe &
$T$ [K] &
$\lambda_{\mathrm{warm}}$ [nm] &
$\lambda_{\mathrm{kalt},1}$ [nm] &
$\lambda_{\mathrm{kalt},2}$ [nm] &
$x$ &
$E_D+E_A$ [meV]\\
\midrule
1127 & 293.15 & 705.0 & -- & -- & 0.226 & -- \\
1127 & 88.75 & -- & 672.1 & 690.5 & -- & 49 \\
\midrule
87063 & 293.15 & 651.0 & -- & -- & 0.328 & -- \\
87063 & 88.25 & -- & 621.0 & 628.0 & -- & 23 \\
\bottomrule
\end{tabular}
\end{table}

Insgesamt zeigen die kalten Spektren, dass bei tieferen Temperaturen zusätzliche Strukturen sichtbar
werden, die bei Raumtemperatur thermisch verbreitert oder überlagert sind. Die Verschiebung der
Hauptmaxima zu kürzeren Wellenlängen bestätigt qualitativ die Vergrößerung der Bandlücke bei sinkender
Temperatur. Probe 87063 besitzt aufgrund des größeren Aluminiumanteils durchgehend die größere
Bandlücke und damit die kürzeren Emissionswellenlängen, während für Probe 1127 die Störstellenenergie
gut mit einer Silizium-Dotierung übereinstimmt.
\section{Zusammenfassung}

Der Gittermonochromator wurde erfolgreich mit Natriumlinien kalibriert. Die Abweichungen der Linienpositionen vom Literaturwert lagen unter \SI{0.14}{\nano\meter}, die gemessenen Halbwertsbreiten knapp oberhalb der nominellen instrumentellen Breite, sodass die Auflösung für die LED- und Lumineszenzspektren ausreichend war.

Die LED-Messungen zeigten typisches Verhalten: Die weiße LED kombiniert einen blauen Halbleiteremitter bei \SI{470.0}{\nano\meter} mit einer breiten Leuchtstoffkonversionsbande bei \SI{545.0}{\nano\meter}. Für die direkten Halbleiter wurden Exziton-Bindungsenergien bestimmt (GaP: \SI{26}{\milli\electronvolt}, Ga(As,P): \SI{48}{\milli\electronvolt}). Die rote Ga(As,P)-Diode mit Maximum bei \SI{655.0}{\nano\meter} lies auf einen Phosphoranteil von $x\approx0.414$ schließen und bleibt damit im direkten Bereich ($x<x_s\approx0.434$).

Die infrarote GaAs-Diode zeigte bei \SI{899.0}{\nano\meter} mit \SI{1.379}{\electronvolt} ein Maximum, das sehr gut mit einer Si-Donator-Akzeptor-Rekombination erklärt wird ($E_g-E_D-E_A\approx\SI{1.379}{\electronvolt}$, mit $E_D\approx\SI{5.48}{\milli\electronvolt}$ und $E_A\approx\SI{40.86}{\milli\electronvolt}$).

Bei den $Ga_{1-x}Al_xAs$-Proben zeigten sich beim Abkühlen auf flüssige-Stickstoff-Temperatur Verschiebungen zu kürzeren Wellenlängen, qualitativ konsistent mit der Bandlückenvergrößerung bei sinkender Temperatur. Aus den Raumtemperaturmaxima ergaben sich Aluminiumanteile von $x_{1127}\approx0.226$ und $x_{87063}\approx0.328$. Probe 1127 zeigte bei Tieftemperatur eine Störstellenenergie von etwa \SI{49}{\milli\electronvolt}, die gut mit einer Si-Dotierung vereinbar ist. Für Probe 87063 ließ sich eine belastbare Störstellenenergie nicht bestimmen, da beide beobachteten Peaks ($\SI{621}{\nano\meter}$ und $\SI{628}{\nano\meter}$) entweder bandkantennah liegen oder vermutlich auf Meßartefakte zurückgehen.
